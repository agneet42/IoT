...
obj = csv.reader(open("loc_sen.csv","r"))
...
for file in final_array:
	in_txt = csv.reader(open("%s.txt" %file, "r"), delimiter = ' ')
	out_csv = csv.writer(open("%s.csv" %file, 'w'))
	out_csv.writerows(in_txt)

w = csv.writer(open("sen_time.csv","w"))
for file in final_array:
	...
	for row in obj1:
		...
		if(flag == 1):
			s = row[0]
			s1 = s[0:2]
			w.writerow([sensor,s1])

mod = csv.reader(open("sen_time.csv","r"))

all_val = []
for obj in mod:
	all_val.append([obj[0],obj[1]])



write1 = csv.writer(open("sen_time1.csv","w"))
write2 = csv.writer(open("sen_time_weights.csv","w"))
for val in arr:
	time = [0]*25
	for val1 in all_val:
		if(val1[0] == val):
